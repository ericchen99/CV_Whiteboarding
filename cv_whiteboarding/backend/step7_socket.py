...
client.connect('localhost:8000/tmp')
client.send(b'python connected')
is_done = False
x = threading.Thread(target=thread_function, args=(client, is_done,))
x.start()
...
while(1):
    ...
    if contours and cv2.contourArea(max(contours,
        ...
        client.send(b'sending variables')
...
def thread_function(client, is_done):
    print("Starting Thread", is_done)
    while True:
